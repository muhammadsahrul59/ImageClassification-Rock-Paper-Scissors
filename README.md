# ImageClassification : Rock-Paper-Scissors ✨

## Project Description
Welcome to the Rock, Paper, Scissors image classification project, where cutting-edge techniques and meticulous design converge to achieve exceptional model accuracy. This project boasts advanced features to enhance model generalization, efficient image processing, and robust training strategies.

## Key Features
- Implemented image augmentation techniques for enhanced model generalization. Utilized ImageDataGenerator for
efficient image processing.
- Divided the dataset into training and validation sets with a 40% validation set size.
- Completed model training within a time frame of 30 minutes. Achieved a model accuracy exceeding 99%
- Implemented four advanced techniques: Callback, Checkpoint, Early Stopping, and Dropout

## Run Google Colab
```
- Download the file above
- open https://colab.research.google.com/
- Upload the file ImageClassification.ipynb
```

or you can open it with this Google Colab Link below :

[![](https://img.shields.io/badge/Google%20colab-Open-FFC000.svg?style=for-the-badge&logo=data:image/png;base64,)](https://colab.research.google.com/drive/1_Xf7qwZCaSfhmDKFNm1qJBKa5QXPJPq2?usp=sharing)


Feel free to experiment, provide feedback, and contribute to the project's enhancement. Your engagement is valued! 🙏
